...
{
    public class UIScreenIndicator : MonoBehaviour
    {
        ...
        protected virtual void Update()
        {
            ...
            if(0 <= screenPos.x && screenPos.x <= screenW && 0 <= screenPos.y && screenPos.y <= screenH)
            {
                ...
                if(!hideIfNotInScreen)
                {
                    ...
                    {
                        viewPos = Core.Misc.MathUtils.CalculateRayScreenIntersection(new Vector2(viewW, viewH), viewPos);
                    }
            ...
            else
            {
                ...
                RectTransformUtility.ScreenPointToLocalPointInRectangle(viewArea, screenPos, uiCamera, out var viewPos);
                //交点计算
                var intersectPos = Core.Misc.MathUtils.CalculateRayScreenIntersection(new Vector2(viewW, viewH), viewPos);
