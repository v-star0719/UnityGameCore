...
{
    //多个3阶贝塞尔曲线组合。为了区别连接是平滑的，上一个曲线的终点和下一个曲线的起点重合，且他们相邻的控制点在同一直线上。
    ...
    public class BezierCombineCurveData : CurveData
    {
        public override Vector3 GetPos(float ratio)
        {
            ...
            f = f - i;
            i = i * 3;
            return MathUtils.Bezier3(points[i], points[i + 1], points[i + 2], points[i + 3], f);
        }
        ...
        public override List<Vector3> ToSegments()
        {
            ...
            for (int i = 0; i < points.Count && i + 3 < points.Count; i += 3)
            {
                ...
                {
                    rt.Add(MathUtils.Bezier3(points[i], points[i + 1], points[i + 2], points[i + 3], j * step));
                }
            }
            return rt;
        }
        ...
        void DrawCurve(Vector3 p0, Vector3 p1, Vector3 p2, Vector3 p3, int smooth)
        {
            ...
            {
                var end = MathUtils.Bezier3(p0, p1, p2, p3, i * step);
